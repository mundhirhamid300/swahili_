...
<form method="POST" action="{{ $purpose==='registration' ? route('register.otp.verify') : route('password.otp.verify') }}">@csrf
...
<form method="POST" action="{{ $purpose==='registration' ? route('register.otp.resend') : route('password.otp.resend') }}" class="text-center mt-3">@csrf<button class="btn btn-link">Resend OTP</button></form>
</x-auth-split>
@endsection
